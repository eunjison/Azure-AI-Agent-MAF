...
# 3. 의존성 패키지 설치 - 다음 중 하나 선택

# 옵션 A: pip install
curl -Ls https://astral.sh/uv/install.sh | bash
uv pip install -r pyproject.toml

# 옵션 B: pyproject.toml 사용 (개발 환경)
pip install -e .
...
├── 🔐 .env                            # 환경변수 (Azure OpenAI 키 설정, Git 무시)
└── 📓 notebooks/                      # Jupyter 실습 노트북
    ├── level-0/                       # 기초 환경 설정
    │   └── 00-setup-environment.ipynb
    └── ...
```
...
- ✅ `AzureCliCredential` 또는 `DefaultAzureCredential` 사용
...
# 해결방법 3: 특정 버전 명시
pip install agent-framework==1.0.0b260123 --pre
```
